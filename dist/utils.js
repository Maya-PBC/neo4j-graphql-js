...
function cypherDirectiveArgs(
  ...
  if (isFederatedOperation) {
    ...
    federatedOperationParams = _objectSpread({}, requiredData, {}, params);
...
var isRelationshipUpdateMutation = function isRelationshipUpdateMutation(
  _ref2
) {
...
function innerFilterParams(filters, neo4jTypeArgs, paramKey, cypherDirective) {
  var temporalArgNames = neo4jTypeArgs
    ? neo4jTypeArgs.reduce(function(acc, t) {
...
var getQueryArguments = function getQueryArguments(
  resolveInfo,
  isFederatedOperation
) {
...
var buildNeo4jTypeCypherParameters = function buildNeo4jTypeCypherParameters(
  _ref15
) {
      ...
      if ((0, _isArray['default'])(param)) {
        ...
        for (; paramIndex < count; ++paramIndex) {
          ...
          if (neo4jTypeParam.formatted) {
            ...
            if (paramKey) params[paramKey][paramName] = _formatted;
            else params[paramName] = _formatted;
          } else {
        ...
        paramStatements.push(
          ''
            ...
            .concat(paramKey ? ''.concat(paramKey, '.') : '')
            .concat(paramName, ' | ')
            .concat(neo4jTypeConstructor, '(value)]')
        );
      } else {
        ...
        if (neo4jTypeParam.formatted) {
          if (paramKey) params[paramKey][paramName] = _formatted2;
          else params[paramName] = _formatted2;
        } else {
          params = serializeNeo4jIntegers({
            paramKey: paramKey,
            fieldTypeName: fieldTypeName,
...
var serializeNeo4jIntegers = function serializeNeo4jIntegers(_ref16) {
  var paramKey = _ref16.paramKey,
    ...
    neo4jTypeParam =
      _ref16$neo4jTypeParam === void 0 ? {} : _ref16$neo4jTypeParam,
  ...
  (0, _keys['default'])(neo4jTypeParam).forEach(function(paramFieldName) {
    ...
    if (
      fieldTypeName === _graphql.GraphQLInt.name &&
      (0, _isInteger['default'])(neo4jTypeParam[paramFieldName])
    ) {
      var serialized = _neo4jDriver['default']['int'](
        neo4jTypeParam[paramFieldName]
      );
...
var getQueryCypherDirective = function getQueryCypherDirective(
  resolveInfo,
  isFederatedOperation
) {
  if (resolveInfo.fieldName === '_entities' || isFederatedOperation) return;
...
var getOuterSkipLimit = function getOuterSkipLimit(first, offset) {
  return ''
    .concat(offset > 0 ? ' SKIP toInteger($offset)' : '')
    .concat(first > -1 ? ' LIMIT toInteger($first)' : '');
};
...
var isSpatialDistanceInputType = function isSpatialDistanceInputType(_ref25) {
  var _ref25$filterOperatio = _ref25.filterOperationType,
    filterOperationType =
      _ref25$filterOperatio === void 0 ? '' : _ref25$filterOperatio;
...
var neo4jTypePredicateClauses = function neo4jTypePredicateClauses(
  filters,
  variableName,
  fieldArguments,
  parentParam
  ...
  return fieldArguments.reduce(function(acc, t) {
    ...
    if (neo4jTypeParam) {
      ...
      if (neo4jTypeParam[_types.Neo4jTypeFormatted.FORMATTED]) {
        ...
        acc.push(
          ''
            .concat(variableName, '.')
            .concat(argName, ' = ')
            .concat(decideNeo4jTypeConstructor(type), '($')
            .concat(
...
var getInterfaceDerivedTypeNames = function getInterfaceDerivedTypeNames(
  schema,
  interfaceName
) {
  var implementingTypeMap = schema._implementations
    ? schema._implementations[interfaceName]
    : {};
  var implementingTypes = [];

  if (implementingTypeMap) {
    implementingTypes = (0, _values['default'])(implementingTypeMap).map(
      function(type) {
        return type.name;
      }
    );
